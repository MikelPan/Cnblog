...
yum install -y wget
#2、下载tar包
wget -c https://www.python.org/ftp/python/3.8.5/Python-3.8.5.tgz -P /usr/local/src
#3、解压文件
tar zxvf /usr/local/src/Python-3.8.5.tgz -C /usr/local/src
...
cd /usr/local/src/Python-3.8.5 && ./configure --prefix=/usr/local/python
make && make install
#6、更换系统python版本
