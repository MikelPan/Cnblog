...
```console
$ helm install <helm-repo>/prometheus-node-exporter
```
...
```console
$ helm install --name my-release <helm-repo>/prometheus-node-exporter
```
...
$ helm install --name my-release \
  --set serviceAccount.name=node-exporter  \
    <helm-repo>/prometheus-node-exporter
```

Alternatively, a YAML file that specifies the values for the above parameters can be provided while installing the chart. For example,

```console
$ helm install --name my-release -f values.yaml <helm-repo>/prometheus-node-exporter
```
